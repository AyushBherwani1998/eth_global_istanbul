...
class CurveGridProvider {
  ...
  Future<HSMWalletsModel> fetchHSMWallets() async {
    const kHSMWalletsPath = "hsm/wallets";
    try {
      final response = await networkClient.getAsync<Map<String, dynamic>>(
        kHSMWalletsPath,
      );
      return HSMWalletsModel.fromJson(response);
    } catch (e, _) {
      rethrow;
    }
  }

  Future<HSMCreateWallet> createNewHSMWallet(String keyName) async {
    const kCreateNewHSMWallet = "hsm/key/new";
    final body = {
      "clientID": dotenv.env["AZURE_CLIENT_ID"],
      "keyName": keyName,
      "vaultName": dotenv.env["AZURE_VAULT_NAME"],
    ...
    try {
      ...
      return HSMCreateWallet.fromJson(response);
    } catch (e, _) {
      rethrow;
    }
  }
